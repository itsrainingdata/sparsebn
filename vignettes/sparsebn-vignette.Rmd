...
On the left, we use the `plotDAG` method, which uses some sensible defaults for plotting large graphs. On the right, we use the `plot` method to use an organized circular layout and change some graphical parameters. The `plot` method allows the user to use the `igraph`, `network`, or `graph` packages for visualization. See `?setPlotPackage` for more details.

For comparison, let's plot the original pathfinder graph; note that the plot on the right makes it easier to this to the previous plot.
```{r, fig.width = 6, fig.height = 6}
par(mfrow = c(1,2), oma = rep(0,4))
...
Assuming unit influences (i.e. $\beta_{ij}=1$ if $\beta_{ij}\ne 0$) and unit variances (i.e. $\omega_j^2=1$ for all $j$), we can then compute $\Sigma$ by using the identity $\Sigma = (I-B)^{-T}\Omega(I-B)^{-1}$:
